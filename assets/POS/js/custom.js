...
$(document).ready(function(){
    ...
    $(document).on('click','.single_item',function(){
        ...
        $('#edit_pricing').on('click',function(){

            var menu_id = $(this).attr('id');
            var products = item_id;
            console.log(item_id)
            if(products!=""){
            	get_pricing_for_edit(products);
            }
        });
    })
...
/*	$(document).on('mouseleave','.header_part_left_left .outlet_holder .outlet_holder_moving',function(){
	...
	$('#given_amount_input').on('keyup',function(e){
		...
		if (e.which != 110 && !(e.which<=109 && e.which>95) && e.which != 190 && e.which != 16 && e.which != 53 && e.which != 8 && e.which != 0 && (e.which < 48 || e.which > 57)) {
			$(this).val('');
		}
	...
	$('#pay_amount_invoice_input').on('keyup',function(e){
		...
		paid_amount = ($(this).val()!="")?$(this).val():0;
		var total_payable = $('#finalize_total_payable').html();
		var total_change = (parseFloat(total_payable)-parseFloat(paid_amount)).toFixed(2);
		$('#due_amount_invoice_input').val(total_change);
	});
...
/*	$(document).on('mouseenter','.header_part_left_left .outlet_holder .outlet_holder_moving',function(){
			...
			$.ajax({
				...
				success:function(response) {
					...
					if(order_type==1){
						...
						order_type_name = 'Comer aca';
					}else if(order_type==2){
						order_number = 'B '+order_object.sale_no;
						order_type_name = 'Para llevar';
    ...
    $('#close_customer_item_modal').on('click',function(){
		$('#products_name').css('border','1px solid #B5D6F6');
		$('#products_pricing').css('border','1px solid #B5D6F6');
		reset_on_modal_close_or_edit_pricing();
	});
	...
	$('#print_kot_modal').on('click',function(){
		...
		if(order_details_id!=""){
			...
			order_details_id_array.forEach(function(entry) {
                ...
                var kot_item_note = $('#item_for_note'+entry)


				items_info += '{"kot_item_name":"'+kot_item_name+'", "kot_item_qty":"'+kot_item_qty+'", "kot_item_note":"'+kot_item_note+'"'; // <span id="item_note_table_'+item_id+'">'+note+'</span>
				var modifiers = '';
				var m = 1;
  ...
  //           })
    ...
    $('#edit_pricing_products').on('click',function(){
		callValidationAjax();
		var customer_id = $('#products_id').val();
		var customer_name = $('#products_name').val();
		var customer_price = $('#products_pricing').val();
		var error = 0;
		...
		if(customer_name==""){
			$('#products_name').css('border','1px solid red');
			error++;
		}

		if(customer_price==""){
			$('#products_pricing').css('border','1px solid red');
			error++;
		}
		if(error!=0){
			return false;
		...
		$.ajax({
			url:base_url+"Sale/add_products_by_ajax",
			method:"POST",
			data:{
				customer_id:customer_id,
				customer_name: customer_name,
			...
			success:function(response) {
				if(response>0){
					$.ajax({
						url:base_url+"Sale/get_all_products",
						method:"GET",
						success:function(response) {
							response = JSON.parse(response);
...
function calculate_create_invoice_modal(){
	...
	var given_amount = ($('#given_amount_input').val()!="")?$('#given_amount_input').val():0;
	var change_amount = (parseFloat(total_payable)-parseFloat(given_amount)).toFixed(2);
...
function reset_on_modal_close_or_edit_pricing(){
	$('#customer_item_modal').fadeOut('500');
    $('#products_id').val('');
	$('#products_name').val('');
	$('#products_pricing').val('');
}
			...
			for (var key in response.items) {
				...
				draw_table_for_kot_modal += '<div class="single_kot_row fix" id="kot_for_item_'+this_item.id+'">';
					...
					draw_table_for_kot_modal += '<div class="kot_content_column fix floatleft kot_item_name_column">';
						draw_table_for_kot_modal += this_item.menu_name+'<br>Nota:'+this_item.menu_note;
					draw_table_for_kot_modal += '</div>';
...
function get_pricing_for_edit(id){
	$.ajax({
		url:base_url+"Sale/get_pricing_ajax",
		method:"POST",
		data:{
			id : id,
		},
		success:function(response) {
			...
			console.log(response);
			// $('#customer_id_modal').val(response.id);
            $('#products_id').val(response.id);
			$('#products_name').val(response.name);
			$('#products_pricing').val(response.sale_price);
			// $('#customer_email_modal').val(response.email);
			// $('#customer_dob_modal').val(response.date_of_birth);
			// $('#customer_doa_modal').val(response.date_of_anniversary);
			// $('#customer_delivery_address_modal').val(response.address);

			$('#customer_item_modal').fadeIn();
		},
		error:function(){
			console.log("Error al Editar Precio");
		}
	});
}
