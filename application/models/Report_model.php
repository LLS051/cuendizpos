...
class Report_model extends CI_Model {
    ...
    public function todaySummaryReport($selectedDate) {
        ...
        //purchase report
        $this->db->select('sum(grand_total) as total_purchase_amount');
        $this->db->from('tbl_purchase');
        if ($selectedDate != '') {
            $this->db->where('date =', $selectedDate);
        } else {
        ...
        //end purchase report
        //Sales report
        $this->db->select('sum(paid_amount) as total_sales_amount,sum(vat) as total_sales_vat'); //paid amount because total payable adds all the number including credits and royalties and those are not sales are lost
        $this->db->from('tbl_sales');
        if ($selectedDate != '') {
            $this->db->where('sale_date =', $selectedDate);
        } else {
...
FROM tbl_ingredients i WHERE del_status='Live' AND i.company_id= '$company_id'  ORDER BY i.name ASC")->result();
    ...
    public function saleReportByMonth($startMonth = '', $endMonth = '', $user_id = '') {
        if ($startMonth || $endMonth || $user_id):
            $outlet_id = $this->session->userdata('outlet_id');
            $this->db->select('sale_date,sum(paid_amount) as total_payable');
            $this->db->from('tbl_sales');
    ...
    public function saleReportByDate($startDate = '', $endDate = '', $user_id = '') {
        if ($startDate || $endDate || $user_id):
            ...
            //$this->db->where_in("payment_method_id", $number);
            $query_result = $this->db->get();
            $result = $query_result->result();
            return $result;
        endif;
    ...
    public function saletarjetreport($startDate = '', $endDate = '', $user_id = '') {
        if ($startDate || $endDate || $user_id):
            $outlet_id = $this->session->userdata('outlet_id');
