...
//dddddddddddddddddddddddddddddddddddddddddddddddddddd
    ...
    public function top_ten_customer($start_date, $end_date) {
        $outlet_id = $this->session->userdata('outlet_id'); 
        $this->db->select('sum(tbl_sales.paid_amount) as total_payable, tbl_sales.customer_id, tbl_customers.name, tbl_sales.payment_method_id, tbl_sales.sale_date');
    ...
    public function supplier_payable($first_day_this_month, $last_day_this_month) {
        $outlet_id = $this->session->userdata('outlet_id');
        $this->db->select('sum(grand_total) as due, supplier_id, name, date, tbl_purchase.date');
    ...
    public function comparison_sale_report($start_date, $end_date) {
        $outlet_id = $this->session->userdata('outlet_id');
        $query = $this->db->query("select year(sale_date) as year, month(sale_date) as month, sum(paid_amount) as total_amount from tbl_sales WHERE `sale_date` BETWEEN '$start_date' AND '$end_date' AND outlet_id='$outlet_id' group by year(sale_date), month(sale_date)");
        return $query->row();
    }
    ...
    public function customer_regalia() {
        $outlet_id = $this->session->userdata('outlet_id');
        $this->db->select('sum(due_amount) as due_amount, customer_id, name');
        $this->db->from('tbl_sales');
        $this->db->join('tbl_customers', 'tbl_customers.id = tbl_sales.customer_id', 'left');
