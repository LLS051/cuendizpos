...
function getTableName($table_id) {
    $CI = & get_instance();
    $information = $CI->db->query("SELECT * FROM tbl_orders_table where `id`='$table_id'")->row();
    return $information->name;
}
...
function getPurchaseIngredients($id) {
    ...
    if (!empty($purchase_ingredients)) { 
        ...
        $pur_ingr_all .= "<b># Ingrediente Cant /Precio por Unid/ Precio total</b><br>";
        foreach ($purchase_ingredients as $value) { 
            $pur_ingr_all .= $key ."- ". getIngredientNameById($value->ingredient_id)." Cant:".$value->quantity_amount ." /". $value->unit_price ." /". $value->total."<br>";
            $key++;
