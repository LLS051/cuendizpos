...
class Sale extends CI_Controller {
    ...
    public function __construct() {
        ...
        if($this->Common_model->isOpenRegister($user_id,$outlet_id)==0){
            $this->session->set_flashdata('exception_3', '¡El registro no está abierto, Ingrese su saldo inicial!');
    ...
    function add_products_by_ajax(){
        $customer_id = $this->input->post($this->security->xss_clean('customer_id'));
        $data['name'] = trim(htmlspecialchars($this->input->post($this->security->xss_clean('customer_name'))));
        $data['sale_price'] = trim(htmlspecialchars($this->input->post($this->security->xss_clean('customer_price'))));
        if($customer_id>0 && $customer_id!=""){
            $this->db->where('id', $customer_id);
            $this->db->update('tbl_food_menus', $data);
        }else{
            $this->db->insert('tbl_food_menus', $data);
        }
        echo 1 ;
    }
    ...
    function get_all_products(){;
        $data1 = $this->db->query("SELECT * FROM tbl_food_menus")->result();
        echo json_encode($data1);
    }
    ...
    public function get_pricing_ajax()
    {
        $id = $this->input->post('id');
        $item_info = $this->Sale_model->getItemInfoById($id);
    ...
    public function update_order_status_ajax()
    {
        ...
        if($close_order=='true'){
            ...
            if($is_just_cloase){
                ...
                $order_status = array('paid_amount' =>  $paid_amount, 'due_amount' => $due_amount, 'order_status' => 3,'payment_method_id'=>$payment_method_type,'close_time'=>date('H:i:s'), 'given_amount'=> $given_amount, 'change_amount' => $change_amount);
            }
    ...
    public function getTotalPaySum($date){
        $user_id = $this->session->userdata('user_id');
        $outlet_id = $this->session->userdata('outlet_id');
        $getTotalPaySum = $this->Sale_model->getTotalPaySum($user_id,$outlet_id,$date);
        return $getTotalPaySum->amount;
    ...
    public function getSaleInStripeSum($date){
        $user_id = $this->session->userdata('user_id');
        $outlet_id = $this->session->userdata('outlet_id');
    ...
    public function sumPaypalsumStripe(){
        $opening_date_time = $this->getOpeningDateTime();
        $credit = $this->getSaleInPaypalSum($opening_date_time);
        $regalia = $this->getSaleInStripeSum($opening_date_time);
        $final = $credit + $regalia;
        return $final;
    }
    ...
    public function registerDetailCalculationToShow(){
        ...
        $register_detail = array(
            ...
            'sale_in_stripe' => $this->getSaleInStripeSum($opening_date_time),
            'total_payable'  => $this->getPayableAomountSum($opening_date_time),
            'sumPay' => $this->getSaleInPaypalSum($opening_date_time),
            'sumStripe' => $this->getSaleInStripeSum($opening_date_time),
           'sumPayStripe' => $this->sumPaypalsumStripe(),
           //'only_paypal' => $this->onlypay()
    ...
    public function getBalance(){
        $opening_date_time = $this->getOpeningDateTime();
        $balance = $this->getOpeningBalance()+$this->getPayableAomountSum($opening_date_time)+$this->getCustomerDueReceiveAmountSum($opening_date_time);
        return  $balance;
    }
    ...
    public function closeRegister()
    {
        ...
        $outlet_id = $this->session->userdata('outlet_id');
        $opening_date_time = $this->getOpeningDateTime();
        $payment_method_json = '{<br>';
        $payment_method_json .= '"Efectivo": '.$this->getSaleInCashSum($opening_date_time).',';
        $payment_method_json .= '<br>"Credito": '.$this->getSaleInPaypalSum($opening_date_time).',';
        $payment_method_json .= '<br>"Tarjeta": '.$this->getSaleInCardSum($opening_date_time).'';
        $payment_method_json .= '<br>"Regalia": '.$this->getSaleInStripeSum($opening_date_time).'';
        $payment_method_json .= '<br>}';
        $changes = array(
